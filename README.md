...
Интерфейс пользователя построен на inline-кнопках: они цветные (`style`) и со
значками-кастом-эмодзи (`icon_custom_emoji_id`, набор RestrictedEmoji, требует
Telegram Premium у владельца бота). Навигация идёт редактированием одного
сообщения (`edit_text`), чтобы не засорять чат. Меню зависит от статуса подписки:

- Подписка активна: «Моя подписка» (дни, ID подписки → «Продлить» / «Подключение»)
  и «Поддержка». В «Подключении» доступность серверов (✅/❌/❓) и кнопки серверов,
  тап по которым копирует ссылку-подписку в буфер (`copy_text`).
- Подписки/пробного не было: «Оформить подписку» (тарифы + правила + промо «приведи
  друга») и «Поддержка». Пробный доступ (3 дня) показывается только если им ещё не
  пользовались и подписка никогда не оформлялась.

Тарифы:
- 1 месяц — 175 ₽;
- 6 месяцев — 850 ₽ (выгода 200 ₽);
- 1 год — 1600 ₽ (выгода 500 ₽);
- 3 дня — бесплатный пробный доступ (один раз на аккаунт).

Оплата: реквизиты для ручного перевода, подтверждение текстом/фото/документом,
уведомление после подтверждения администратором.
...
### Единая подписка SubHub

Бот продолжает создавать и продлевать клиентов непосредственно в 3x-ui, но
пользователю выдаёт одну постоянную ссылку SubHub со всеми странами и
протоколами. Настройте внутренний admin API:

```env
SUBHUB_URL=http://127.0.0.1:8080
SUBHUB_ADMIN_TOKEN=replace-with-the-same-token-as-subhub
SUBHUB_TIMEOUT_SECONDS=15
SUBHUB_RESOLVE_ATTEMPTS=4
```

После оплаты, trial, привязки, ручного provisioning и успешного pending update
бот запускает SubHub sync. Экран «Подключение» при необходимости повторяет
resolve несколько раз и показывает одну кнопку копирования. Пятиминутный poll
SubHub остаётся резервным механизмом.

`SUBHUB_ADMIN_TOKEN` является секретом. Не передавайте его пользователям и не
логируйте ответы resolve: они содержат персональную subscription URL. Если бот
и SubHub находятся на разных хостах, ограничьте `/admin/` по IP и используйте
HTTPS.
...
SERVER_HEALTH_POLL_SECONDS=60   # период фоновой проверки серверов (0 — отключить)
```

Бот в фоне раз в `SERVER_HEALTH_POLL_SECONDS` секунд проверяет доступность всех
панелей 3x-ui (пробует login) и сохраняет статус в `servers.is_online` /
`servers.last_checked_at`. Статус показывается пользователю в разделе «Подключение».

Значки на кнопках берутся из набора кастом-эмодзи RestrictedEmoji и заданы в
`app/bot/emoji.py`. Для их отображения у владельца бота должен быть Telegram
Premium; иначе кнопки останутся цветными, но без значков.
...
(раздел «Моя подписка» → «Подключение», тап по серверу копирует ссылку).
Проверить/перевыпустить доступ вручную: `/provision <telegram_id>`,
посмотреть конфигурацию серверов: `/servers`.

### Перенос пользователей, существовавших до бота

Если на панели уже есть клиенты, заведённые вручную до внедрения бота, их можно
привязать к боту и продлевать как обычных:

1. Пользователь запускает бота (`/start`) — у него появляется `public_id`.
2. Админ смотрит существующих клиентов панели: `/panelclients <server_id>`.
3. Привязывает клиента: `/bind <server_id> <email> <telegram_id>`.

При привязке сохраняются UUID/secret, `subId` и срок действия клиента. Если у
клиента был `subId`, он становится `public_id` пользователя (чтобы ссылка-подписка
совпадала). Дальнейшие продления (`/extend`, оплата) идут по уже существующему
клиенту, а не создают нового. Клиент с конечным сроком сразу синхронизируется на
все сервера; «бессрочные» клиенты синхронизируются после первого `/extend`.
...
- `/sharing all` — полный список VPN-клиентов и число уникальных IP за 24 ч, 1 ч и 7 дней;
...
- `/renameserver <server_id> <новое название>` — изменить название сервера без удаления;
...
- `/delinbound <server_id> <inbound_id>` / `/clearinbounds <server_id>` — удалить настроенные inbound'ы;
- `/provision <telegram_id>` — создать/обновить клиента пользователя на всех серверах;
- `/panelclients <server_id>` — показать клиентов, уже существующих на панели;
- `/bind <server_id> <email> <telegram_id>` — привязать существующего клиента панели к пользователю бота и включить продление.
